...
class RBF(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        X_X = np.atleast_2d(X_X)
        X_Z = np.atleast_2d(X_Z)

        length_scale = _check_length_scale(X, self.length_scale)
        if Y is None:
            dists = pdist(X_X / length_scale, metric='sqeuclidean')
            K = np.exp((-.5) * dists)
            dists2 = pdist(X_Z / length_scale, metric='sqeuclidean')
            ...
            Y_X = np.atleast_2d(Y_X)
            Y_Z = np.atleast_2d(Y_Z)

            if eval_gradient:
                raise ValueError(
                    "Gradient can only be evaluated when Y is None.")
            dists = cdist(X_X / length_scale, Y_X / length_scale,
                          metric='sqeuclidean')

            dists2 = cdist(X_Z / length_scale, Y_Z / length_scale,
                          metric='sqeuclidean')
            K = np.exp(-.5 * dists)*np.exp(-.5 * dists2)
